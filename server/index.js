...
class DragonRebootServer {
  ...
  async checkFileContent() {
    ...
    try {
      if (this.verboseLogging) {
        await this.logMessage(`Poll #${this.pollCount}: Checking GCP file existence...`, 'DEBUG');
      }
      
      const [exists] = await this.file.exists();
      if (!exists) {
        await this.logMessage(`File ${this.fileName} does not exist in bucket ${this.bucketName}`, 'WARN');
        this.errorCount++;
        return null;
      }
      ...
      await this.logMessage(`Poll #${this.pollCount}: Content="${contentString}" (${content.length} bytes, ${downloadTime}ms)`, 'INFO');
      
      if (this.verboseLogging) {
        await this.logMessage(`File metadata: Updated=${metadata.updated}, Size=${metadata.size}, ETag=${metadata.etag}`, 'DEBUG');
      }
      ...
      this.errorCount++;
      await this.logMessage(`Error reading file from GCP (attempt ${this.errorCount}): ${error.message}`, 'ERROR');
  ...
  async resetTriggerFile() {
    try {
      await this.file.save('safe', {
        metadata: {
          contentType: 'text/plain'
        }
      });
      await this.logMessage('Reset trigger file to "safe"', 'INFO');
    } catch (error) {
      await this.logMessage(`Error resetting trigger file: ${error.message}`, 'ERROR');
    }
  }
  ...
  async getHealthStatus() {
    ...
    return {
      ...
      nextPollIn: this.isRunning ? `${Math.ceil(this.pollInterval / 1000)}s` : 'N/A',
      memory: {
  ...
  async executeReboot() {
    ...
    exec(`sudo bash "${scriptPath}"`, async (error, stdout, stderr) => {
      if (error) {
        await this.logMessage(`Error executing reboot script: ${error.message}`, 'ERROR');
        return;
      }
  ...
  async start() {
    ...
    try {
      ...
      await this.logMessage(`❌ Failed to connect to GCP: ${error.message}`, 'ERROR');
      process.exit(1);
    }
    ...
    setInterval(async () => {
      if (this.verboseLogging) {
        ...
        await this.logMessage(`Health check: ${JSON.stringify(health, null, 2)}`, 'DEBUG');
      }
    }, 5 * 60 * 1000);
  }
