...

{
  ...
  systemd.services.dragon-reboot = {
    ...
    serviceConfig = {
      ...
      ExecStart = "${pkgs.nodejs}/bin/node /opt/dragon-reboot/server/index.js";
      WorkingDirectory = "/opt/dragon-reboot";
      EnvironmentFile = "/opt/dragon-reboot/.env";
...
```bash
# Copy files manually
sudo mkdir -p /opt/dragon-reboot
sudo cp -r ./* /opt/dragon-reboot/
sudo chown -R root:root /opt/dragon-reboot

# Install dependencies
cd /opt/dragon-reboot
...
```bash
# Build and run
docker build -t dragon-reboot .
docker run -d --name dragon-reboot \
  --privileged \
  -v /var/log:/var/log \
  --env-file .env \
  dragon-reboot
...
command_args="/opt/dragon-reboot/server/index.js"
command_user="root"
